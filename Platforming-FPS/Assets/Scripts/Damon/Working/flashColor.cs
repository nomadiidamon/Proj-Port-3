...
public class flashColor : MonoBehaviour
{
    ...
    [Header("-----Currently only two colors supported-----")]
    ...
    public void flashOne()
    {
        ...
        float changeValue = Mathf.PingPong(Time.time * flashSpeed, 1.0f);


        Color modelColor = Color.Lerp(originalColor, color1, changeValue);
        rend.material.color = modelColor;

        Color lightColor = Color.Lerp(originalColor, color1, changeValue);
        myLight.color = lightColor;
        myLight.intensity = Mathf.Lerp(minIntensity, maxIntensity, changeValue);

        rend.sharedMaterial.SetColor("_EmissionColor", lightColor * myLight.intensity);

        if (flashRange)
        {
            currentRange = Mathf.Lerp(minRange, maxRange, changeValue);
            myLight.range = currentRange;
        }
    ...
    public void flashTwo()
    {
        ...
        Color modelColor = Color.Lerp(color1, color2, changeValue);
        rend.material.color = modelColor;

        Color lightColor = Color.Lerp(color1, color2, changeValue);
        myLight.color = lightColor;
